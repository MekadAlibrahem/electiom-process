...
if(empty($id)){
    ...
    echo '<div class="alert alert-danger" role="alert"> يجب ادخال نوع الحساب اولا 
    </div>';
}else{
    ...
    if($query_select){
        // تم تنفيذ الاستعلام عن صحة الرقم الوطني
            if(mysqli_num_rows($query_select)>0){ 
                // الرقم الوطني صحيح 
                // القيام بالتعديل
                $edit_type_account = "UPDATE `useraccount` SET `TypeAccountID`='$type' WHERE  `NationalNumber` = '$id'";
